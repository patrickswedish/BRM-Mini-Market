...
const AboutUs: React.FC<AboutUsProps> = ({ t }) => {
  return (
    <section className="bg-black py-16 md:py-32 px-4 md:px-8 border-t-4 md:border-t-8 border-white overflow-hidden relative">
      ...
      <div className="max-w-[1440px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-20 items-center">
        <div className="lg:col-span-7 flex flex-col items-start order-2 lg:order-1">
          ...
          <h2 className="font-oswald text-4xl xxs:text-5xl sm:text-6xl md:text-7xl lg:text-8xl text-white leading-[0.9] md:leading-none uppercase mb-4">
            {t.h2} <br className="hidden sm:block" />
            <span className="text-[#ffff00]">{t.h2_yellow}</span>
          </h2>
          
          <h3 className="text-base xxs:text-lg md:text-3xl font-black text-white/90 uppercase tracking-tight mb-6 md:mb-8 italic">
            {t.h3}
          </h3>
        ...
        <div className="lg:col-span-5 relative order-1 lg:order-2 w-full max-w-md mx-auto lg:max-w-none">
          <div className="relative">
            <div className="border-[4px] md:border-[12px] border-white shadow-[8px_8px_0px_rgba(168,28,33,0.5)] md:shadow-[15px_15px_0px_rgba(168,28,33,0.5)] overflow-hidden bg-zinc-900 group aspect-[3/4] lg:aspect-auto">
              <img 
